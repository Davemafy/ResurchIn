...
export function ResearchStory() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      const ratio = Math.max(0, Math.min(0.999, -rect.top / travel));
      const next = Math.floor(ratio * storyData.length);
      setPhaseChanging(true);
      window.clearTimeout(timer);
      timer = window.setTimeout(() => {
        setActive(next);
        setPhaseChanging(false);
      }, reducedMotion.current ? 0 : 170);
    };
  ...
  const choose = (index: number) => {
    ...
    if (window.innerWidth > 720 && sectionRef.current) {
      ...
      window.scrollTo({
        top: sectionRef.current.offsetTop + (travel * index) / storyData.length + 2,
